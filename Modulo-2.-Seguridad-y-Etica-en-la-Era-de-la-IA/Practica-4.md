...
4. Ingresa a Copilot, ChatGPT o Gemini y escribe el siguiente prompt:
    ```
    Anonimiza el siguiente texto eliminando datos personales identificables como nombres, RFC, 
    correos, teléfonos y cualquier otro dato que permita identificar a una persona física. 
    Mantén el contexto general del texto. 
...
6. Ahora envía un prompt más específico:
    ```
    Anonimiza el siguiente texto eliminando cualquier dato personal identificable (PII), 
    sustituyendo nombres y datos sensibles por roles genéricos. No elimines información 
    relevante para el análisis organizacional.
...
1. Responde lo siguiente:
    - ¿Qué pasaría si cada equipo decide por su cuenta qué datos "son seguros" para enviar a la IA?
